...
| 公開チャンネルのメンバー管理URL（`/channels/{channel}/members` 系）を叩いた（作成者を含む、誰であっても） | 403 Forbidden（実装時に追記、2026-08-19） | ※設計判断。F-09〜F-11 は**プライベートチャンネル限定の機能**（`features.md`）で、公開チャンネルにはメンバー管理そのものが無い（`data.md` 2-3「公開チャンネルではこれ以上行が増えない」）。上の「権限が無い操作」と同じ扱いにした。404にしないのは、公開チャンネルは誰にでも見えていて存在を隠す必要が無く、404にすると逆に「そのIDのチャンネルは無い」という誤った情報を返すことになるため |
...
| 返信（`parent_message_id` が入っているメッセージ）を親に指すスレッドのURLを叩いた（`GET /channels/{channel}/messages/{返信}/thread`／`POST /channels/{channel}/messages/{返信}/replies`） | 404（実装時に追記、2026-08-19） | ※設計判断。返信は1段までで返信はスレッドの親になれない（`questions.md` Q-07、`data.md` 2-4「DB制約ではなくアプリ側でチェックする」）ため、そのURLは**存在しない**という扱いにした。403（権限が無い操作）にしないのは、誰であっても・どんな権限でも到達できないURLで、権限の話ではないため。UI側では返信にスレッドを開く導線を出していないが、クライアント側の制御だけに頼らずサーバ側でも確認する |
