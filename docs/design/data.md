...
補足（実装時に追記、2026-08-19）: **暫定**（`questions.md` Q-05、回答なし）。メンバーを外す操作で消すのは `channel_user` の該当行だけで、`messages` には一切触れない。外された人が過去に書き込んだメッセージは投稿者名も本文もそのまま残り、残ったメンバーには見え続ける。Q-05 の回答が来たら見直す。
...
補足（実装との突き合わせで追記、2026-08-20）: 表の数え方をそろえた。実物と突き合わせたときに `-` と `R` の境目が人によってぶれたため、基準をここに置く。

- **入力チェックの重複確認・存在確認も `R` に数える**（F-01のメールアドレス重複、F-05のチャンネル名重複、F-10の「すでにメンバーです」）。SELECTが1本走る以上、読み取りとして表に出す
- **URLパラメータをモデルに引き当てる読みも `R` に数える**（F-11の`{user}`）
- **判定のために読むものも `R` に数える**（F-13・F-14が「チャンネルが見えるか」を確かめるための `channels`・`channel_user`）。F-12が同じ判定を `R` として書いているので、そちらに揃えた
- **どの機能でも共通に走る読みは、行に書かずここにまとめる。**ログイン後の全画面で、認証ミドルウェアが `users` を1件読み、共通レイアウトのサイドバー（`AppServiceProvider` の View Composer が `Channel::visibleTo()` を呼ぶ）が `channels` と `channel_user` を読む。これを機能単位の行に毎回書くと、その機能自身の判定と区別が付かなくなるため入れていない

| ID | 機能 | users | channels | channel_user | messages |
|:--|:--|:--|:--|:--|:--|
| F-01 | 新規登録 | R, C（メールアドレスの重複確認 → 作成） | - | - | - |
| F-02 | ログイン | R | - | - | - |
| F-03 | ログアウト | - | - | - | - （4章「埋まらない行」参照） |
| F-04 | チャンネル一覧表示 | R（作成者名の表示） | R | R（Q-04：非メンバーの private を除外） | - |
| F-05 | チャンネル作成 | - | R, C（チャンネル名の重複確認〔Q-03〕 → 作成） | C（作成者を自動追加、spec §3-2） | - |
...
| F-07 | チャンネル編集 | - | R, U | - | R（削除確認カードの「メッセージ{n}件・返信{m}件」を数える〔`screens.md` 3-6〕。カードはSC-06に同居していて `GET .../edit` で読むので、F-08ではなくこの行に置いた） |
| F-08 | チャンネル削除 | - | R, D（確認入力とチャンネル名の照合 → 削除） | D（CASCADE） | D（CASCADE。0章の論理削除とは別枠の物理削除） |
| F-09 | メンバー一覧表示 | R（氏名・メール表示） | R | R | - |
| F-10 | メンバー追加 | R（Q-06：氏名/メール完全一致で検索） | R（private・作成者チェック） | R, C（`screens.md` 4章「すでにメンバーです」の判定に既存行の確認が必要） | - |
| F-11 | メンバー削除 | R（URLの`{user}`を引き当てる） | R（作成者チェック） | D | - |
| F-12 | メッセージ投稿 | - | R（書き込み可否判定） | R（private時） | C |
| F-13 | メッセージ編集 | R（編集状態で描き直すときの投稿者名） | R（チャンネルが見えるかの判定） | R（private時） | R, U（本人確認のうえ `body`・`edited_at`更新） |
| F-14 | メッセージ削除 | - | R（チャンネルが見えるかの判定） | R（private時） | R, U（本人確認のうえ `deleted_at`のみ更新。`body`は変更しない） |
...
| F-19 | 公開API：公開チャンネルのメッセージ一覧 | R（投稿者の表示名をレスポンスに含める。※設計判断、`permissions-api.md`で確定） | R（対象channelがpublicか確認） | - | R（**`deleted_at IS NULL`を明示的に条件へ**、Q-11の回答どおり削除済みは非表示。返信は含めない＝**`parent_message_id IS NULL`も明示的に条件へ**。**暫定**：Q-12は回答なし、2026-08-19の実装で決めた〔4章〕） |
...
- ~~**F-19（公開APIのメッセージ一覧）が返信を対象に含むか**~~: `questions.md` Q-12は「回答なし」のままだが、実装を止められないため **暫定で「含めない」と決めた**（2026-08-19、実装単位(7)）。`Channel::messagesForPublicApi()` に `parent_message_id IS NULL` を入れ、元メッセージだけを返す。Q-12の想定（「返信も、元メッセージと区別できる形で含める想定ですが」）とは**逆**の判断で、`permissions-api.md` 3章のレスポンス例に返信を区別するキーが無いことに合わせた。クライアントの回答ではないため、回答が来たら見直す（3章F-19行に反映済み）。
