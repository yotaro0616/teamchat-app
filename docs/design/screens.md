...
**編集中の「返信N件」について**（実装単位(5)で追記、2026-08-19）: 上の追記に2点を足す（※設計判断）。**編集中のメッセージには「返信N件」リンクを出さない**。`mockup/channel-show.html` でこのリンクは本文（`msg__body`）か削除済みの枠（`msg__deleted-note`）の直後にしか現れず、編集中はそのどちらも `textarea` に置き換わって存在しないため。また、**対象が返信のときの「やめる」の戻り先は `/channels/{channel}` ではなく親のスレッドURL**にする（`permissions-api.md` 2章。編集状態を出しているのがスレッドパネルの中なので、そこへ戻す）。
...
**画面の作り方について**（実装時に追記、2026-08-19）: SC-08 専用のビューは作らず、**SC-05 のビュー（`channels/show.blade.php`）にスレッドの情報が渡されたときだけ3列目のパネルを足す**（※設計判断）。理由: `mockup/thread.html` の `main` 側は `channel-show.html` と完全に同じで、SC-08 は「SC-05 ＋ 右パネル」そのものだから。別ビューにすると、メッセージ一覧・日付区切り・投稿欄が二重管理になる。3列への切り替えは `mockup/thread.html` と同じ `app__body app__body--thread`（SC-05 は `app__body`）で、共通レイアウトに既定値付きのフックを2つ（枠のクラス名と、`main` の後ろに `aside` を差し込む口）足して実現する。返信を編集中の表示（`permissions-api.md` 2章の `GET .../edit`）も、同じ口にパネルを出して1件だけ編集状態にする。

**返信が0件のときの見せ方について**（実装時に追記、2026-08-19）: 上の表の「返信件数見出し」に **「返信0件」と出すだけ**にする（※設計判断）。`design-guide.md` §4「空・0件」の破線枠は使わない。理由: パネルには常に元メッセージがあって画面としては「空」ではなく、見出しの型「返信{n}件」に n=0 を素直に入れれば足りるため。返信0件のスレッドは、SC-05 のホバー操作の「返信」から開いたときに出る（SC-05 の「返信N件」リンクは1件以上のときだけ出るため。3-5）。

**閉じる（×）について**（実装時に追記、2026-08-19）: 上の表では「ボタン」だが、実装では**リンク（`/channels/{channel}` へ戻る `<a>`）**にする（※設計判断）。見た目は `mockup/thread.html` と同じ `icon-btn`・`title="閉じる"` のまま。理由: 4章で決めた JavaScript の使用範囲は文字数カウンタとボタンの活性・非活性だけで、画面遷移は JS に頼らない。この操作はサーバへ送るものが何も無いので、フォーム（`<button>`）にする必要が無い（ログアウトを `<button>` にしたのは `POST` が要るからで、判断の筋は同じ）。

**返信欄の入力名について**（実装単位(5)で追記、2026-08-19）: 返信欄の `name` は `body` ではなく **`reply_body`** にする（※設計判断）。この画面は SC-05 の投稿欄をそのまま残したうえに返信欄を足すので、同じページに本文の入力欄が2つ並ぶ。両方を `body` にすると、片方が入力チェックで弾かれたときに `old()` がどちらの入力か区別できず、返信の本文が本流の投稿欄にも復元される。上限と必須の決まりは本流と同じ（`spec` §5-1）で、エラー文を出さない方針も変えない（4章）。詳細は `permissions-api.md` 2章。

**パネルの中のホバー操作について**（実装時に追記、2026-08-19）: 上の「ホバー操作もSC-05と同じ」には次の但し書きが付く（※設計判断）。**返信アイコンはパネルの中には出さない**（元メッセージにも返信にも）。返信への返信はできない（Q-07）ので、そこから開ける先が無いため。**編集・削除は3-5 と同じ条件で出す**（自分の投稿かつ削除済みでないときだけ）。あわせて、**出すアイコンが1つも無いときは `msg__acts` の枠自体を描かない**（他人の返信にホバーすると空の白枠だけが出てしまうため）。また、SC-05 の「返信N件」リンクもパネルの中には出さない（`mockup/thread.html` の元メッセージに `thread-link` が無い）。
...
**実装時に追記（2026-08-19）**: 表に無い5点を実装時に決めた。

- **結果の並び順**: 新しい順（`created_at` 降順、同時刻は `id` 降順）。※設計判断。この章に並び順の明記が無く、`mockup/search.html` の3行が `2026/08/17`→`08/14`→`08/12` の降順で並んでいる実例を根拠にした
- **`q` が空／未指定でアクセスしたとき**: エラー文を出さず、フォームだけを表示する。結果件数見出し・結果行・0件メッセージのいずれも出さない（「まだ検索していない」状態）。※設計判断。この章は「空欄なら検索ボタンを押せない」しか定めておらず、サーバ側の応答（直接URLを叩いた場合）は未定義だったため、他の未定義箇所（4章「削除確認の入力がチャンネル名と一致しないとき」）と同じ「エラー文を増やさず素の状態に戻す」方針に倣った
- **検索ボタンの活性/非活性の実現方法**: `public/js/app.js` の文字数カウンタ機構を流用するが、検索欄には表示用カウンタが無い（モックアップに無い）。既存JSは表示先（`output`）が無いと何もしない作りだったため、`data-counter-submit`（送信/実行ボタンの参照）だけを持ち `data-counter`（表示先）を持たない要素でも disabled 切り替えを行うよう、JSを最小限拡張した
- **ヘッダーの検索窓の実装形**: `layouts/app.blade.php` の空 `<div class="topbar__search"></div>` を、`GET` フォーム（`name="q"`）に差し替えた。見た目は既存の `.search-input` のクラスをそのまま使い、中の `<span>` を `<input>` に替えている。モックアップはどのページでも装飾のspanのままで実例が無いため、この組み方を実装時に決めた
- **検索結果行のチャンネル表示**: `channels/partials/sig.blade.php` は使わず、`search.html` の実物どおり `<span class="result__ch">` 1つに記号（`#`／鍵アイコン）と名前をまとめて書いた。`sig` パーシャルは記号だけを別spanで返す作りで、`search.html` の構造と一致しないため（`blade.md`「モックアップの構造が違うところでは partial を使わない」）
